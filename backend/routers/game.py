...
def start_game(body: GameStartRequest, user=Depends(current_user)):
    try:
...
def next_question(session_id: str, user=Depends(current_user)):
    question = game_engine.get_next_question(session_id, user["id"])
    if question is None:
...
def answer_question(body: GameAnswerRequest, user=Depends(current_user)):
    try:
        result = game_engine.submit_answer(
            body.session_id, body.word_id, body.chosen, body.time_ms, user["id"]
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    ...
    update_word_progress(
        ...
        known_on_type_mastery=body.known_on_type_mastery, user_id=user["id"],
    )
    return GameAnswerResponse(**result)


@router.post("/skip")
def skip_word(session_id: str, word_id: int, user=Depends(current_user)):
    try:
        result = game_engine.skip_word(session_id, word_id, user["id"])
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    return result
